...
## Clean up alt scenarios
stats_new$OM<-ifelse(stats_new$OM=='','base',substr(stats_new$OM,1,3))
### set Blim depending on OM
stats_new$Blim <- sapply(stats_new$OM, function(x) {
  switch(x, "base" = 107297,"M01" = 90094, "M03" = 133650)})
...
stats_new$iavTAC_long <- foreach(x = res_list, .packages = "FLCore",
                                 .combine = "c") %dopar% {
                                   iav(object = window(x@tracking["metric.is"], start = 2028), summary_all = median)
                                 }
stats_new$iavTAC_short <- foreach(x = res_list, .packages = "FLCore",
                                  .combine = "c") %dopar% {
                                    iav(object = window(x@tracking["metric.is"], start = 2018, end = 2023), 
                                        summary_all = median)
                                  }
stats_new$iavTAC_medium <- foreach(x = res_list, .packages = "FLCore",
                                   .combine = "c") %dopar% {
                                     iav(object = window(x@tracking["metric.is"], start = 2023, end = 2028), 
                                         summary_all = median)
                                   }
### SSB
...
### change HCR name for A*
stats$HCR[stats$Btrigger==149098]<-"A*"
...
grid <- function(dat, HCR = "A",
  ...
  p1 <- ggplot() +
    ...
    scale_x_continuous(breaks = c(seq(from = 110, to = 280, by = 10))) +
...
dir.create("output/runs/pok/1000_21/plots")
...
grid(dat = stats %>%
       filter(HCR == "A" & BB == FALSE & TACconstr == FALSE &
                ...
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM =="base"), 
     HCR = "A", time = "long", add_risk1 = FALSE, highlight_max = TRUE)
ggsave(filename = "output/runs/pok/1000_21/plots/grid_A_long.png", 
...
grid(dat = stats %>%
       filter(HCR == "A" & BB == FALSE & TACconstr == FALSE &
                ...
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),       
     HCR = "A", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_A_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "A" & BB == FALSE & TACconstr == FALSE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2) &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "A", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_A_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
...
ggsave(filename = "output/runs/pok/1000_21/plots/grid_B_long.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "B" & BB == FALSE & TACconstr == FALSE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2) &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"), 
     HCR = "B", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_B_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "B" & BB == FALSE & TACconstr == FALSE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2) &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"), 
     HCR = "B", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_B_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
...
ggsave(filename = "output/runs/pok/1000_21/plots/grid_C_long.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "C" & BB == FALSE & TACconstr == FALSE & 
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "C", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_C_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "C" & BB == FALSE & TACconstr == FALSE & 
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"), 
     HCR = "C", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_C_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
...
grid(dat = stats %>%
       filter(HCR == "A" & BB == TRUE & TACconstr == TRUE &
                ...
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"), 
     HCR = "A", time = "long", add_risk1 = FALSE, highlight_max = TRUE)
ggsave(filename = "output/runs/pok/1000_21/plots/grid_AD_long.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "A" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "A", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_AD_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "A" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "A", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_AD_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")

### AD1 - AD with the alternative TAC constraint (HCRoption==7)
grid(dat = stats %>%
       filter(HCR == "A1" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "A1", time = "long", add_risk1 = FALSE, highlight_max = TRUE)
ggsave(filename = "output/runs/pok/1000_21/plots/grid_AD1_long.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "A1" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "A1", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_AD1_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "A1" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "A1", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_AD1_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
...
grid(dat = stats %>%
       filter(HCR == "B" & BB == TRUE & TACconstr == TRUE &
                ...
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "B", time = "long", add_risk1 = FALSE, highlight_max = TRUE)
ggsave(filename = "output/runs/pok/1000_21/plots/grid_BE_long.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "B" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "B", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_BE_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "B" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "B", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_BE_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
...
grid(dat = stats %>%
       filter(HCR == "C" & BB == TRUE & TACconstr == TRUE &
                ...
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "C", time = "long", add_risk1 = FALSE, highlight_max = TRUE)
ggsave(filename = "output/runs/pok/1000_21/plots/grid_CE_long.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "C" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "C", time = "medium")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_CE_medium.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
grid(dat = stats %>%
       filter(HCR == "C" & BB == TRUE & TACconstr == TRUE &
                Ftrgt %in% round(seq(0, 1, 0.01), 2)  &
                Btrigger %in% round(seq(110000,280000,10000),-4) & OM == "base"),
     HCR = "C", time = "short")
ggsave(filename = "output/runs/pok/1000_21/plots/grid_CE_short.png", 
       width = 30, height = 20, units = "cm", dpi = 300, type = "cairo")
...
plot_stk <- function(stats, OM_ = "", 
                     HCR_ = "B", Ftrgt_ = 0.4,
                     Btrigger_ = 210000, TACconstr_ = FALSE, BB_ = FALSE,
                     probs = c(0.05, 0.25, 0.5, 0.75, 0.95),
                     path_out = "output/runs/pok/1000_21/",
                     path_in = "input/pok/1000_21/", 
                     file_in = ifelse(OM_ == 'base',"base_run.rds",
                                      ifelse(OM_ == 'M01', "M01_base_run.rds","M03_base_run.rds")),
                     path_res = "output/runs/pok/1000_21/plots/stock_plots/",
                     save_plot = TRUE,
                     get_history = TRUE, overwrite_catch_history = FALSE,
                     yr_start = 2017.5,
                     Blim = 107297, MSYBtrigger = 149098,
                     Flim = 0.620, Fmsy = 0.363,
...
### double-checking reading correct data for OM
plot_stk(stats = stats, OM_ = "M01", HCR_ = "A", Ftrgt_ = 0.36, 
         Btrigger_ = 230000, TACconstr_ = TRUE, BB_ = TRUE, iters_plot = 1:5,
         overwrite_catch_history = TRUE,file_in = "M01_base_run.rds",
         path_res = "output/runs/pok/1000_21/plots/tmp")
### A, B, C, AD, BE, CE, A1D

combs <- data.frame(name = c("F0","A*", "A", "B", "C", "AD", "BE", "CE", "A1D", "B_precautionary","BE_precautionary", "AD_precautionary"),
                    HCR = c("F0","A*", "A", "B", "C", "A", "B", "C", "A1","B","B","A"),
                    BB = c(rep(FALSE, 5), rep(TRUE,4),FALSE,TRUE,TRUE),
                    TACconstr = c(rep(FALSE, 5), rep(TRUE,4),FALSE,TRUE,TRUE),
                    Btrigger = c(0, 149098, 250000, 200000, 250000, 210000, 220000,
                                 230000, 230000, 210000, 210000,230000),
                    Ftrgt = c(0, 0.363, 0.35, 0.39, 0.35, 0.41, 0.39, 0.36, 0.36, 0.38, 0.36,0.37)) 					
combs <- rbind(cbind(combs, OM = "base"),
               cbind(combs, OM = "M01"),
               cbind(combs, OM = "M03")
)

####
combs<-combs[-c(10:13, 22:25, 34:36),] #### select only the scenarios that have been read in from the output files ####
#combs<-combs[11,]	
...
### plot with any five iterations
lapply(X = split(combs, seq(nrow(combs))), FUN = function(i) {
  plot_stk(stats = stats, OM_ = i$OM, HCR_ = i$HCR, Ftrgt_ = i$Ftrgt, 
           Btrigger_ = i$Btrigger, TACconstr_ = i$TACconstr, BB_ = i$BB, 
           overwrite_catch_history = TRUE, iters_plot = 100:105,
           path_in = "input/pok/1000_21/")
})
...
combs <- data.frame(name = c("F0","A*", "A", "B", "C", "AD", "BE", "CE", "A1D"),
                    OM = c("base"),
                    HCR = c("F0","A*", "A", "B", "C", "A", "B", "C", "A1"),
                    BB = c(rep(FALSE, 5), rep(TRUE,4)),
                    TACconstr = c(rep(FALSE, 5), rep(TRUE,4)),
                    Btrigger = c(0, 149098, 250000, 200000, 250000, 210000, 220000,
                                 230000, 230000),
                    Ftrgt = c(0, 0.363, 0.35, 0.39, 0.35, 0.41, 0.39, 0.36, 0.36), 					
                    scenario = 0) 					
...
combs2$name <- factor(combs2$name, levels = c("F0","A*", "A", "B", "C", "AD", "BE", "CE", "A1D"))
...
stats_full <- function(data) {
  ...
  combs_full$name <- factor(combs_full$name, 
                            levels = c("F0","A*", "A", "B", "C", "AD", "BE", "CE", "A1D"))
  return(combs_full)
}
...
p_ssb_long <- ggplot(data = combs_base[combs_base$key == "ssb_long", ], 
  ...
  geom_boxplot() + theme_bw() + ylim(0, NA) +
  geom_hline(aes(yintercept = value_median), colour = "red", alpha = 0.5) +
  coord_cartesian(ylim = c(0, 3.5e+06)) +
...
p_ssb_medium <- ggplot(data = combs_base[combs_base$key == "ssb_medium", ], 
  ...
  geom_boxplot() + theme_bw() + ylim(0, NA) +
  geom_hline(aes(yintercept = value_median), colour = "red", alpha = 0.5) +
  coord_cartesian(ylim = c(0, 2.5e+06)) +
...
plot_grid(p_catch_long, p_risk1_long, p_risk3_long, p_iav_long, p_ssb_long,
          align = "hv")
ggsave(filename = paste0("output/runs/pok/1000_21/plots/baseOM_stats/", 
...
### add: 0.9 & 1.1 * Ftrgt
###      Fmsylower, Fmsyupper
combs <- data.frame(name = rep(c("A", "B", "C", "AD", "BE", "CE", "A1D"), each = 5),
                    HCR = rep(c("A", "B", "C", "A", "B", "C", "A1"), each = 5),
                    BB = rep(c(rep(FALSE, 3), rep(TRUE, 4)), each = 5),
                    TACconstr = rep(c(rep(FALSE, 3), rep(TRUE, 4)), each = 5),
                    Btrigger = rep(c(250000, 200000, 250000, 210000,
                                     220000, 230000, 230000), each = 5),
                    Ftrgt = c(0.35 * c(0.9, 1, 1.1), 0.210, 0.536,
                              0.39 * c(0.9, 1, 1.1), 0.210, 0.536,
                              0.35 * c(0.9, 1, 1.1), 0.210, 0.536,
                              0.41 * c(0.9, 1, 1.1), 0.210, 0.536,
                              0.39 * c(0.9, 1, 1.1), 0.210, 0.536,
                              0.36 * c(0.9, 1, 1.1), 0.210, 0.536,
                              0.36 * c(0.9, 1, 1.1), 0.210, 0.536),
...
p_ssb_long <- ggplot(data = combs_dat[combs_dat$key == "ssb_long", ], 
  ...
  geom_boxplot(show.legend = TRUE, size = 0.25, outlier.size = 0.3,
               position = position_dodge2(preserve = "single")) +
  coord_cartesian(ylim = c(0, 1.25e+6)) +
  theme_bw() + ylim(0, NA) +
  labs(x = "", y = "long-term SSB [t]") +
...
ggsave(filename = paste0("output/runs/pok/1000_21/plots/baseOM_stats_combs/", 
...
### alternative OMs
### select maximum yield combinations
combs_alt <- data.frame(name = c("A*","A", "B", "C", "AD", "BE", "CE", "A1D"),
                        HCR = c("A*","A", "B", "C", "A", "B", "C", "A1"),
                        BB = c(rep(FALSE, 4), rep(TRUE,4)),
                        TACconstr = c(rep(FALSE, 4), rep(TRUE,4)),
                        Btrigger = c(149098,250000, 200000, 250000, 210000,
                                     220000, 230000, 230000),
                        Ftrgt = c(0.363,0.35, 0.39, 0.35, 0.41, 0.39, 0.36, 0.36),
                        scenario = 0)
combs_alt <- rbind(cbind(combs_alt, OM = "base"),
                   cbind(combs_alt, OM = "M01"),
                   cbind(combs_alt, OM = "M03"))
...
p_catch_long <- ggplot(data = combs_alt[combs_alt$key == "catch_long", ], 
  ...
  theme_bw() + ylim(0, NA) +
  labs(x = "", y = "long-term catch [t]") +
  coord_cartesian(ylim = c(0, 5.5e+05)) +
  scale_y_continuous(labels = function(x) format(x, scientific = TRUE, digits = 1),
                     limits = c(0, NA))
...
p_catch_short <- ggplot(data = combs_alt[combs_alt$key == "catch_short", ], 
  ...
  theme_bw() + ylim(0, NA) +
  labs(x = "", y = "short-term catch [t]") +
  coord_cartesian(ylim = c(0, 3.2e+05)) +
  scale_y_continuous(labels = function(x) format(x, scientific = TRUE, digits = 2),
                     limits = c(0, NA))
...
plot_grid(plot_grid(p_catch_long, p_risk1_long, p_risk3_long, p_iav_long,
                    ...
                    align = "hv"),
          get_legend(p_ssb_long), nrow = 2, rel_heights = c(1, 0.1))
ggsave(filename = paste0("output/runs/pok/1000_21/plots/altOMs_stats/", 
...
### ------------------------------------------------------------------------ ###
df <- foreach(OM = c("", "M01_", "M03_"),
              .combine = rbind) %do% {
                res <- readRDS(paste0("output/runs/pok/1000_21/",OM,
                                      "HCR-A_Ftrgt-0.35_Btrigger-250000_TACconstr-FALSE_BB-FALSE.rds"))
                res_input <- readRDS(paste0("input/pok/1000_21/",OM,"base_run.rds"))$om@stock
...
df$OM<-ifelse(df$OM=='','base',as.character(df$OM))
...
### get optimized A
stkA_file <- stats %>% filter(OM == "base" & Ftrgt == 0.35 & Btrigger == 250000 &
                                TACconstr == FALSE & BB == FALSE & HCR == "A")
### get simulated SSB
ssbA_new <- ssb(readRDS(paste0("output/runs/pok/1000_21/", 
                               stkA_file$file))@stock)
### get historical SSB
ssbA <- ssb(readRDS("input/pok/1000_21/base_run.rds")$om@stock)
### combine
...
ggsave(filename = paste0("output/runs/pok/1000_21/plots/stock_plots/", 
