...
## installed FLa4a, FLCore, mse, ggplotFL
...
### grabbing stock object from the website - only works on newer version of SAM code, o/w build from scratch
sei <- fitfromweb(NS_saithe_2018_rerun)
pok_conf <- loadConf(sei$data, "./conf/model.cfg")

### downloaded code from cod MSE github modified for saithe
...
if (length(args) >= 2 || (exists("iters") && exists("years"))) {

  ### extract arguments
  for (i in seq_along(args)) eval(parse(text = args[[i]]))

  if( !exists("iters") || !exists("years") )
    forceSTOP <- TRUE
  ...
  if( !exists("m_criteria") ) {
    ...
    if(m_criteria %in% c("01", "02", "03")) {
      if(m_criteria == "01") { m_criteria <- "M01"; pok_stk <- pok_stkM01 }
      else if(m_criteria == "02") { m_criteria <- "" }
      else if(m_criteria == "03") { m_criteria <- "M03"; pok_stk <- pok_stkM03}
    } else {
...
if(forceSTOP){
  stop("You must provide at least iters and years. m_criteria is optional.")
}
...
### number of it 10erations/replicates
n_iter <- iters
### number of years
n_years <- years
### last data year
...
### add uncertainty ####
### ------------------------------------------------------------------------ ###
### first approach: use variance-covariance matrix (see Work Plan notes)
...
## SAM_uncertainty gets the covariance matrix of all model parameters and creates random values based on estimation and covar sim states
## output = stock.n = stock.n, harvest = harvest,survey_catchability = catchability, catch_sd = catch_sd,survey_sd = survey_sd, proc_error = SdLogN
#sam_initial$logFpar  -- check number for surveys (7)
### add noise to stock
...
# for(i in 1:8){
# print(i);summary(harvest(stk)[i]);summary(harvest(stk_orig)[i])
# }
...
bio_samples <- sample(x = sample_yrs_pos_bio, 
                      size = (n_years + 1) * n_iter, replace = TRUE)
### do the same for selectivity
sel_samples <- sample(x = sample_yrs_pos_sel, 
                      size = (n_years + 1) * n_iter, replace = TRUE)
...
if (isTRUE(verbose)) {
	...
	as.data.frame(FLQuants(fitted = sr@fitted, rec = sr@rec, SSB = sr@ssb)) %>%
	  mutate(age = NULL,
			 year = ifelse(qname == "SSB", year + 3, year)) %>%
...
### use means of sampled values for projection period
  ...
  yearMeans(catch.wt(stk_oem)[, ac(bio_sample_yrs)])
landings.wt(stk_oem)[, ac(proj_yrs)] <- 
  yearMeans(landings.wt(stk_oem)[, ac(bio_sample_yrs)])
discards.wt(stk_oem)[, ac(proj_yrs)] <- 
  yearMeans(discards.wt(stk_oem)[, ac(bio_sample_yrs)])
stock.wt(stk_oem)[, ac(proj_yrs)] <- 
  yearMeans(stock.wt(stk_oem)[, ac(bio_sample_yrs)])
m(stk_oem)[, ac(proj_yrs)] <- ifelse(m_criteria == "",
		yearMeans(m(stk_oem)[, ac(bio_sample_yrs)]), 0.2)
...
# idx[1] <- calc_survey(stk = stk_fwd, idx = idx[1])
# idx[2] <- calc_survey(stk = stk_fwd, idx = idx[2])
...
set.seed(4)
idx_dev[[1]][] <- unlist(lapply(lapply(uncertainty$survey_cov, "[[", 1),
                                      function(x){
    t(mvrnorm(n = dim(idx_dev[[1]])[2], 
              mu = rep(0, dim(idx_dev[[1]])[1]), 
              Sigma = x))
}))
...
### noise
idx_dev[[2]][] <- stats::rnorm(n = length(idx_dev[[2]]),
                                     mean = 0, sd = idx_dev[[2]])
### exponentiate to get from normal to log-normal scale
...
### check survey
# idx0 <- calc_survey(stk = stk_fwd, idx = idx, biomassindex = init_biomassindex)
...
### check SAM ####
### ------------------------------------------------------------------------ ###
# keep number of iterations small if running on computer
# or if n_iter is small, use n_iter
# n_iter_sam <- 5
# if(n_iter < n_iter_sam) n_iter_sam <- n_iter
# 
# stk_tmp <- window(stk_stf, end = 2018)
# catch.wt(stk_tmp)[,ac(2018)] <- landings.wt(stk_tmp)[,ac(2018)] <-
#   discards.wt(stk_tmp)[,ac(2018)] <- NA
# stk_tmp <- stk_tmp[,,,,, 1:n_iter_sam]
# idx_tmp <- window(idx, end = 2017)
# idx_tmp <- lapply(idx_tmp, FLCore::iter, 1:n_iter_sam)
# 
# save.image("./output/rerun 2018 assessment.RData")
# 
# fit3 <- FLR_SAM(stk = stk_tmp, 
#                 idx = idx_tmp, conf_full = TRUE, conf = pok_conf)
# stk3 <- SAM2FLStock(fit3)
# 
# #png(filename="./output/runs/pok/SAM check end OM.png")
# plot(iter(FLStocks(pok = stk, sim = stk3), 1))
# #dev.off()
...
# pok_conf <- readRDS(file = paste0(input_path, "pok_conf"))
### catch numbers
...
refpts_mse <- list(Btrigger = 149098,
				       Ftrgt = 0.363,
                       Fpa = 0.446,
                       Bpa = 149098,
				       Blim = 107297)
