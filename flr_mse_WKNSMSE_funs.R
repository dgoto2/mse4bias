...
SAM_uncertainty <- function(fit, n = 1000, print_screen = FALSE) {
  ...
  . <- capture.output(sds <- TMB::sdreport(obj = fit$obj, par.fixed = fit$opt$par,
  ...
  stk_template <- FLQuant(dimnames = list(age = min_age:max_age, year = years, iter = 1:n))
  ...
  catchability <- lapply(seq_along(idx_surveys), function(x) {
    ...
    tmp[] <- exp(t(dat[, colnames(dat) == "logFpar", drop = FALSE][, idx_LogFpar[[x]]]))
    return(tmp)
  })
  ...
  ### time-invariant
  ### template
  catch_sd <- FLQuant(dimnames = list(age = dimnames(stock.n)$age, year = "all",  iter = 1:n))
  
  ### index for catch sd (some ages are linked)
  catch_sd_idx <- idxObs[1, ][idxObs[1, ] > -1] + 1
  
  ### extract values
  catch_sd[] <- exp(t(dat[, colnames(dat) == "logSdLogObs", drop = FALSE][, catch_sd_idx]))
  ...
  ### fill
  SdLogN[] <- exp(t(dat[, colnames(dat) == "logSdLogN", drop = FALSE][, idx_SdLogN]))
