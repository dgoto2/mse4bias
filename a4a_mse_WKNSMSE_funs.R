...
SAM_wrapper <- function(stk, idx, tracking,
  ...
  for (i in 1:length(fit)) {
    if (!is.null(fit[i]) & length(fit[i][[1]]$sdrep$estY)>0) {  
      fit[i][[1]]$sdrep$value[1:(ay-fit[i][[1]]$data$years[1])] <- fit[i][[1]]$sdrep$value[1:(ay-fit[i][[1]]$data$years[1])] + log(prop_biasN) ## a bias in stock number at age
      
      fit[i][[1]]$sdrep$estY[1:8] <- fit[i][[1]]$sdrep$estY[1:8] + log(prop_biasN) ## a bias in stock number at age
      fit[i][[1]]$sdrep$estY[9:15] <- fit[i][[1]]$sdrep$estY[9:15] + log(prop_biasF) ## a bias in F at age
      ### store ssb from each ay for the last 6 years of simualtions for the retrospective analysis
      if (max(fit[i][[1]]$data$years)>=2031) {
  ...
  if (ay==2033) {attr(tracking@units, "ssb_2033") <- ssb_sam}
  if (ay==2034) {attr(tracking@units, "ssb_2034") <- ssb_sam}
  if (ay==2035) {attr(tracking@units, "ssb_2035") <- ssb_sam}
  if (ay==2036) {attr(tracking@units, "ssb_2036") <- ssb_sam}
  if (ay==2037) {attr(tracking@units, "ssb_2037") <- ssb_sam}
  if (ay==2038) {attr(tracking@units, "ssb_2038") <- ssb_sam}
